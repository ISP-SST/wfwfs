#ifndef WFWFS_UTIL_STRINGUTIL_HPP
#define WFWFS_UTIL_STRINGUTIL_HPP
/***************************************************************************
 *                                                                         *
...
namespace wfwfs {
    ...
    template <typename T>
    T pop( std::string &line, const char *separator = " \t\n" ) {
        try {
            std::string w = popword( line, separator );
            if( ! w.empty() ) {
                return boost::lexical_cast<T>(w);
            }
    ...
    inline bool isRelative( const boost::filesystem::path &p ) { return isRelative( p.string() ); }
    ...
    */
    ...
    std::string printVariableMap( const boost::program_options::variables_map vm );
...
#endif // WFWFS_UTIL_STRINGUTIL_HPP
