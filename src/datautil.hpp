#ifndef WFWFS_DATAUTIL_HPP
#define WFWFS_DATAUTIL_HPP
/***************************************************************************
 *                                                                         *
...
namespace wfwfs {
    ...
    inline U dg_correct( const T* in, size_t offset, const float* ddPtr, const float* ggPtr, int maxval ) {
        float c = (in[offset] - ddPtr[offset]) * ggPtr[offset];
        if( c < 0 ) return static_cast<U>(0);;
        if( c > maxval ) return static_cast<U>(maxval);
        return static_cast<U>(c);
    }
    

    template <typename T, typename U, typename V>
    inline void dg_correct( const T* __restrict__ in,
                            size_t n,
                                  U* __restrict__ out,
                            const V* __restrict__ ddPtr,
                            const V* __restrict__ ggPtr,
                            int maxval ) {
        for( size_t i(0); i<n; ++i ) {
            double c = (in[i] - ddPtr[i]) * ggPtr[i];
            if( c < 0 ) c = 0.0;
    ...
    template<typename T, typename U=T>
    inline T restrictValue( T val, U min, U max ) {
...
#if defined(__GNUC__)
#  define WFWFS_UNUSED __attribute__ ((unused))
#elif defined(_MSC_VER)
#  define WFWFS_UNUSED __pragma(warning(suppress:4100))
#else
#  define WFWFS_UNUSED
#endif


#endif // WFWFS_DATAUTIL_HPP
