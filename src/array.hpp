#ifndef WFWFS_ARRAY_HPP
#define WFWFS_ARRAY_HPP
/***************************************************************************
 *                                                                         *
...
namespace wfwfs {
    ...
    public:
        ...
        public:
            const_iterator( const Array<T>& a, int64_t pos, int64_t begin, int64_t end ) : pos_( pos ), begin_( begin ), end_( end ), m_ConstArrayPtr( &a ), cdata( a.datablock.get() ) {
                if( a.dense_ ) {
                    incrementor = &wfwfs::Array<T>::const_iterator::fastIncrement;
                    decrementor = &wfwfs::Array<T>::const_iterator::fastDecrement;
                } else {
                    incrementor = &wfwfs::Array<T>::const_iterator::sparseIncrement;
                    decrementor = &wfwfs::Array<T>::const_iterator::sparseDecrement;
        ...
        Array( T* data, S ...sizes ) : dense_(true), begin_(0), end_(0) {
            wrap( data, {static_cast<size_t>( sizes )...} );
        }
        ...
        void copy( Array<U>& out ) const {
            ...
            if( dense_ && out.dense_ ) {
                std::copy( get()+begin_, get()+end_, out.get()+out.begin_ );
                //std::transform(out.ptr(), out.ptr()+nElements_, get()+begin_, out.ptr(), wfwfs::assign<U,T>());
            } else {
                std::transform(out.begin(), out.end(), begin(), out.begin(), wfwfs::assign<U,T>());
            }
        }
        ...
        void assign( const Array<U>& rhs ) {
            if( sameSize( rhs ) ) {
                if( dense_ && rhs.dense_ ) {
                    ...
                    std::transform(get()+begin_, get()+end_, rhs.get()+rhs.begin_, get()+begin_, wfwfs::assign<T,U>());
                } else {
                    std::transform(begin(), end(), rhs.begin(), begin(), wfwfs::assign<T,U>());
                }
            } else {
        ...
        const Array<T>& operator=( const Array<U>& rhs ) {
            ...
            if( dense_ && rhs.dense_ ) {
                //std::transform(rawPtr, rawPtr+nElements_, rhs.get(), rawPtr, wfwfs::assign<T,U>());
                std::copy(rhs.get()+rhs.begin_, rhs.get()+rhs.end_, get()+begin_);
            } else {
                std::transform(begin(), end(), rhs.begin(), begin(), wfwfs::assign<T,U>());           // using the (slow) iterators defined above.
            }
            return *this;
        }
        ...
        const Array<T>& operator+=( const Array<U>& rhs ) {
            if( this->sameSize( rhs ) ) {
                if(dense_ && rhs.dense_) {
                    std::transform(get()+begin_, get()+end_, rhs.get()+rhs.begin_, get()+begin_, wfwfs::add<T,U>());
                } else {
                    std::transform(begin(), end(), rhs.begin(), begin(), wfwfs::add<T,U>());
                }
            }
            else {
        ...
        const Array<T>& operator-=( const Array<U>& rhs ) {
            if( this->sameSize( rhs ) ) {
                if(dense_ && rhs.dense_) {
                    std::transform(get()+begin_, get()+end_, rhs.get()+rhs.begin_, get()+begin_, wfwfs::subtract<T,U>());
                } else {
                    std::transform(begin(), end(), rhs.begin(), begin(), wfwfs::subtract<T,U>());
                }
            }
            else {
                throw std::invalid_argument( "Array:-=  dimensions does not match: " + printArray(dimensions(),"dims")
        ...
        const Array<T>& operator*=( const Array<U>& rhs ) {
            if( this->sameSize( rhs ) ) {
                if(dense_ && rhs.dense_) {
                    std::transform(get()+begin_, get()+end_, rhs.get()+rhs.begin_, get()+begin_, wfwfs::multiply<T,U>());
                } else {
                    std::transform(begin(), end(), rhs.begin(), begin(), wfwfs::multiply<T,U>());
                }
            }
            else {
                throw std::invalid_argument( "Array:*=  dimensions does not match: " + printArray(dimensions(),"dims")
        ...
        const Array<T>& operator/=( const Array<U>& rhs ) {
            if( this->sameSize( rhs ) ) {
                if(dense_ && rhs.dense_) {
                    std::transform(get()+begin_, get()+end_, rhs.get()+rhs.begin_, get()+begin_, wfwfs::divide<T,U>());
                } else {
                    std::transform(begin(), end(), rhs.begin(), begin(), wfwfs::divide<T,U>());
                }
            }
            else {
                throw std::invalid_argument( "Array:/=  dimensions does not match: " + printArray(dimensions(),"dims")
        ...
        template <typename ...S>
        auto reshape(S ...s) -> std::shared_ptr<typename wfwfs::detail::Dummy<T,S...>::dataType> {
            return wfwfs::reshapeArray(datablock.get(),s...);
        }
        template <typename ...S>
        auto reshape(S ...s) const -> std::shared_ptr<const typename wfwfs::detail::Dummy<T,S...>::dataType> {
            return wfwfs::reshapeArray(datablock.get(),s...);
        }
        //@}
        ...
        void setLimits( const std::vector<U>& first, const std::vector<U>& last) {
            ...
            nElements_ = 1;
            for( size_t i = 0; i < nDims_; ++i ) {
                //int64_t tmpLast = wfwfs::bound_cast<int64_t>( dimFirst[i] + last[i], 0, dimLast[i] );
                //int64_t tmpFirst = wfwfs::bound_cast<int64_t>( dimFirst[i] + first[i], 0, dimLast[i] );
    ...
    template <> template <>
    inline const Array<double>& Array<double>::operator=( const Array<wfwfs::complex_t>& rhs ) {
    ...
    template <> template <>
    inline const Array<float>& Array<float>::operator=( const Array<wfwfs::complex_t>& rhs ) {
        if( !sameSize( rhs ) ) this->resize( rhs.currentSizes );
        if(dense_ && rhs.dense_) {
            std::transform(rhs.get()+rhs.begin_, rhs.get()+rhs.end_, get()+begin_, [](complex_t d) { return std::real(d); } );
        } else {
